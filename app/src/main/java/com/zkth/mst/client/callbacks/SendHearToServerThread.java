...
public class SendHearToServerThread extends Thread {
    ...
    public void run() {
        ...
        long timeStemp = PhoneUtils.dateStamp();
        ...
        power[0] = (byte) AppConfig.battery;
        System.arraycopy(power, 0, requestBytes, 80, 1);
        //内存使用
        byte[] mem = new byte[1];

        mem[0] = (byte) Double.parseDouble(AppConfig.ram + "");
        System.arraycopy(mem, 0, requestBytes, 81, 1);
        //cpu使用
        byte[] cpu = new byte[1];
        cpu[0] = (byte) Double.parseDouble(AppConfig.cpu + "");

        System.arraycopy(cpu, 0, requestBytes, 82, 1);
        //信号强度
        byte[] signal = new byte[1];

        signal[0] = (byte) AppConfig.wifi;
